...
class UsageMeter:
    ...
    Thread-safe: the orchestrator runs independent DAG nodes concurrently, all
    sharing one meter, so `record` and the budget reads are guarded by a lock.
    """
    ...
    def record(
        ...
        *,
        ...
        with self._lock:
            ...
            # No provider usage — estimate both sides; flag the run approximate.
            self.estimated = True
